...
<body class="devDesktop no-color chrome home_index type_default desktop " style="<?php if(!empty($_COOKIE['background'])){echo "background: ".$_COOKIE['background'].";";} ?>" onLoad="checkType();">
    ...
    <div class="container">
				...
				$nSecurity = mysqli_num_rows($qSecurity);
					...
					if($nSecurity['wykonawca_id']=='0'){$Locked = false;}else{$Locked=true;}
				if($nSecurity == 0){header("Location: mojeZlecenia");exit;}
				//##############################################################
				...
				if($nRealizacja > 0){
					$sRealizacja = mysqli_fetch_array($qRealizacja);
					...
					if(!empty($korekta_probnaRealizacja)){
						...
						<div style="margin-top: 10px; margin-bottom: 10px; background: #f8f8f8; border-left: 3px solid '; if($korekta_probna_zatwierdzonaRealizacja == "1"){echo "#428400;";}else{echo '#da3070;';} echo ' padding: 15px;">
							...
							<div style="text-align: right;">
								<a href="acceptKorektaProbna-'.$id.'" class="btn btn-primary"><i class="fa fa-check-circle"></i> Zaakceptuj korektę próbną</a>
							</div>
							';
							}
							echo '
					...
					if(!empty($trescRealizacja)){
						...
						errorFIXED("Oczekiwanie na realizację przez wykonawcę");
					}
				}
				?>
        <div class="row">
            <div class="col-12">
				...
				if($nGetOffer > 0){
					...
					if($closedGetOffer != "1"){header("Location: mojeZlecenia");}
