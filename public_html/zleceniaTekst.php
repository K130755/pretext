...
<?php if($jestem_copywriter != "1"){header("Location: notForMe");} ?>
                ...
                <form method="post" accept-charset="utf-8" class="filter on-mobile-hidden" action="filtrujZlecenia">
                    ...
                    <div class="left-filter-items">
                        <div class="form-group filter-cena">
                            ...
                            <div><span>od</span>
                                <input type="text" data-type="float" name="cena_od" value="<?=round($cena_odFiltry);?>" /><span>do</span>
                                <input type="text" data-type="float" name="cena_do" value="<?=round($cena_doFiltry);?>" /><span>/</span>
                ...
                <div class="row zlecenia-lista-content">
                    <div class="col-12">
                        <?php
												...
												$q = mysqli_query($connect,"SELECT * FROM `zlecenia` WHERE id > 0 $filter AND `closed` != '1' AND `typ` = 'nowy' AND (`dla_ulubionych` = '' OR `dla_ulubionych` LIKE '%,".$_SESSION['user_id']."%') ORDER BY `id` $sortowanieFiltry LIMIT $start, $perPage");
												$n = mysqli_num_rows($q);
												if($n > 0){
                                ...
                                <div class="col-12 col-md-12 col-lg-3 zlecenie-lista-actions">
                                    ...
                                    <div class="btn-action"><span class="jak-to-dziala-link" data-toggle="popover-info" data-trigger="hover" data-content="Kliknij w PRZYJMIJ ZLECENIE, aby przejść do realizacji. Od tej pory zlecenie będzie przypisane do Twojego konta. Po akceptacji przez ZLeceniodawcę, zostaniesz poinformowany o tym stosownym komunikatem. Klikając DOŁĄCZ DO KOLEJKI wyrażasz chęć wzięcia udziału w ew. ponownej realizacji tego zlecenia.">Jak to działa?</span>
																		...
																		<?php
																			...
																			if($wykonawca_id != $_SESSION['user_id']){
																			 $nCheckKolejka = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM `kolejka` WHERE `zlecenie_id` = $id"));
																			 if($nCheckKolejka > 0){
																				...
																				if($nCheckKolejka2 > 0){
																					echo '<a href="'; if($moj_typ == "wykonawca"){echo 'joinOffer-'.$id;} else {echo "javascript:;";} echo '" class="btn text-center btn-block btn-primary btn-rezerwuj-zlecenie"><i class="fs-20 fa fa-check-circle"></i> Przejdź do zlecenia</a>';
																				} else {
																					echo '<a href="'; if($moj_typ == "wykonawca"){echo 'setMeUp-'.$id;} else {echo "javascript:;";} echo '" class="btn text-center btn-block btn-primary btn-rezerwuj-zlecenie"><i class="fs-20 fa fa-check-circle"></i> Dołącz do kolejki</a>';
																				}
																			 } else {
																				echo '<a href="'; if($moj_typ == "wykonawca"){echo 'joinOffer-'.$id;} else {echo "javascript:;";} echo '" class="btn text-center btn-block btn-primary btn-rezerwuj-zlecenie"><i class="fs-20 fa fa-check-circle"></i> Przyjmij zlecenie</a>';
																			 }
																			} else {echo '<a href="'; if($moj_typ == "wykonawca"){echo 'joinOffer-'.$id;} else {echo "javascript:;";} echo '" class="btn text-center btn-block btn-primary btn-rezerwuj-zlecenie"><i class="fs-20 fa fa-check-circle"></i> Przejdź do zlecenia</a>';}
																		} ?>
																		</div>
                                </div>
                            </div>
